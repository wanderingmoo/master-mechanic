...
id_to_system = {}
facts.each { |row| id_to_system[row["fact_id"]] = row["system"] }
settings.each { |row| id_to_system[row["setting_id"]] = row["system"] }
gaps.each { |row| id_to_system[row["gap_id"]] = row["system"] }
parts.each { |row| id_to_system[row["part_id"]] = row["system"] }
configuration.each { |row| id_to_system[row["item_id"]] = row["system"] }
...
systems = (facts.map { |row| row["system"] } +
           settings.map { |row| row["system"] } +
           gaps.map { |row| row["system"] } +
...
if options[:format] == "markdown"
  ...
  systems.each do |system|
    system_facts = facts.select { |row| row["system"] == system }
    system_settings = settings.select { |row| row["system"] == system }
    system_gaps = gaps.select { |row| row["system"] == system }
